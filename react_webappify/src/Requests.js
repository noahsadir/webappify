...
export function jsonAPICall(jobID, args, callback) {
  ...
  .then(function(promise) {
    ...
    if (contentType && contentType.indexOf("application/json") !== -1) {
      ...
      return promise.json().then(data => {
        callback(jobID,true,data);
      }).catch(function(error) {
        //Notify listener that request failed with error
        callback(jobID,false,reportError("PROMISE_BROKEN"));
        console.log(error);
      });
    } else {
      return promise.text().then(text => {
        ...
        //Notify listener that request failed with error
        callback(jobID,false,reportError("NOT_JSON"));
  ...
  .catch(function(error) {
    ...
    callback(jobID,false,reportError("NO_FETCH"));
    console.log(error);
  });
}
